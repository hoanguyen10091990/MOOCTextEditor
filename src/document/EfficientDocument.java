...
public class EfficientDocument extends Document {
	...
	private void processText()
	{
		...
		// Start with init efficient documents
		numWords = 0;
		numSentences = 0;
		numSyllables = 0;
		boolean flag = false;
		// Process all token in tokens list
		for (String token: tokens) {
		// Check for each word with isWord
			if(isWord(token)) {
		// If isWord 
				// Add 1 to numWord
				numWords += 1;
				// Call countCyllables and add to numSyllables
				numSyllables += countSyllables(token);
				flag = true;
			} else {
		// If not, means punctuation, add 1 to sentence
				numSentences += 1;
				flag = false;
			}
		}
		if (flag) this.numSentences += 1;
	}
