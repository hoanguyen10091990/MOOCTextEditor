...
public class NearbyWords implements SpellingSuggest {
	...
	// For use in the Optional Optimization in Par t 2.
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		while (queue.size() > 0 && retList.size() < numSuggestions && visited.size() < THRESHOLD) {
			...
			// Get word from queue
			String w = ((LinkedList<String>)queue).remove();
			// Generate list of words away from current word if this word not visited word
			List<String> lstAway = distanceOne(w, true);
			for (String wo : lstAway) {
				if (! visited.contains(wo)) {
					queue.add(wo);
					visited.add(wo);
					if (dict.isWord(wo)) {
						retList.add(wo);
